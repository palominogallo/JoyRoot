...
def write_rootclass( filepath, filename ):
    ...
    fileclass.write( "\tmReader.SetTree(tree);\n" )
    fileclass.write( "\tmNevents = mReader.GetEntries( kTRUE );\n}\n\n" )
    ...
    fileclass.write( "\treturn mReader.SetEntry( entry );\n}\n\n" )
    fileclass.write( "#endif")
    fileclass.close()
...
def write_rootheader( filepath, filename ):
    ...
    fileclass.write( "\t\tvirtual ~%s();\n\n" % filename )
    fileclass.write( "\tpublic:\n" )
    fileclass.write( "\t\tTTreeReader\t\t\tmReader;\n" )
...
def write_makefile( path, dirname, classname  ):
    ...
    filesrcm.write( "CXXFLAGS = -c -g -O2 -Wall -fPIC `root-config --cflags`\n")
    filesrcm.write( "ROOTLDFLAGS =  `root-config --glibs` -lTreePlayer\n" )
    ...
    filehism.write( "CXXFLAGS = -c -g -O2 -Wall -fPIC `root-config --cflags`\n")
    filehism.write( "ROOTLDFLAGS =  `root-config --glibs` -lTreePlayer\n" )
    ...
    fileplom.write( "CXXFLAGS = -c -g -O2 -Wall -fPIC `root-config --cflags`\n")
    fileplom.write( "ROOTLDFLAGS =  `root-config --glibs` -lTreePlayer\n" )
